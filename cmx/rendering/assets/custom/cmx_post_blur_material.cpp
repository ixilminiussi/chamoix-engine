...
void PostBlurMaterial::bind(FrameInfo *frameInfo, Drawable const *)
{
    if (_boundID != _id)
    {
        _pipeline->bind(frameInfo->commandBuffer);

        size_t depthID = _renderSystem->getGBuffer()->getSamplerDescriptorSetIDs()[3];
        size_t ssaoID = _renderSystem->getSSAOBuffers()[0]->getRenderTargets()[0].descriptorSetID;
        frameInfo->commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, _pipelineLayout, 0, 1,
                                                    &(_renderSystem->getSamplerDescriptorSet(depthID)), 0, nullptr);
...
void PostBlurMaterial::initialize()
{
    ...
    createPipeline(renderSystem->getSSAOBuffers()[1]->getRenderPass());
}
